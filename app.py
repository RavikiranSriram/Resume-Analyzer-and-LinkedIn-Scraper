...
def streamlit_config():
    ...
    # title and position
    st.markdown(f'<h1 style="text-align: center;">Resume Analyzer and <br> LinkedIn Scraper</h1>',
...
class resume_analyzer:
    ...
    def summary_prompt(query_with_chunks):
        ...
        query = f''' Create detailed summarization of below resume, after that mention the detailed strengths of below resume and finally conclude them
    ...
    def strength_prompt(query_with_chunks):
        query = f'''write a generic cover letter for a role by explaining in detail about the below resume 
                    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
                    {query_with_chunks}
                    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
                    '''
        ...
        if submit:
            if pdf is not None and openai_api_key != '':
                try:
                    ...
                    st.markdown(f'<h4 style="color: orange;">Cover Letter:</h4>', unsafe_allow_html=True)
                    st.write(strength)
    ...
    def job_title_prompt(query_with_chunks):
        ...
        query = f'''what are the job roles i should apply to on LinkedIn based on the below resume?
...
with st.sidebar:
    ...
    option = option_menu(menu_title='', options=['Summary', 'Cover Letter', 'Weakness and Areas to Improve', 'Job Titles', 'Linkedin Jobs'],
...
elif option == 'Weakness and Areas to Improve':
